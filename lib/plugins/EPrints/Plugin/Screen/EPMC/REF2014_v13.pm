package EPrints::Plugin::Screen::EPMC::REF2014_v13;
...
sub action_enable
{
	...
	# Add the REF2014 UoAs:
	$self->add_ref2014_uoas();

	# Change the labels for UoA's A1 to B9 if necessary:
	$self->update_ref2014_labels();

	# If people are coming from an upgrade, they might need the new UoA 'Z' (added in v1.1)
	$self->add_ref2014_uoa_z();

	# re-commit ref_selection dataset (pre-populate ref_selection.output_type - added in v1.1)
...
sub add_ref2014_uoas
{
	...
	my $test_subject_id = $ds->dataobj( 'ref2014_uoas' );
...
sub add_ref2014_uoa_z
{
	...
	return if( defined $subds->dataobj( 'ref2014_z' ) );
...
sub update_ref2014_labels
{
	...
	my $repo = $self->{repository};
        ...
        my $test_subject = $subds->dataobj( 'ref2014_a1' ) or return;
	...
	{
		my $subject = $subds->dataobj( "ref2014_$id" ) or next;
...
sub recommit_ref_selections
{
	...
	my $benchmark = EPrints::DataObj::REFBenchmark->default( $repo ) or return;
...
sub update_datasets
{
	...
	my $iepm = $repo->dataset( "epm" )->dataobj( 'ref2014' ) or return;

        foreach my $repoid ( $iepm->repositories )
        {
                my $repo2 = EPrints->new->repository( $repoid );
		
		foreach( 'ref_selection', 'ref_circ', 'ref_benchmark', 'ref_environment' )
		{
