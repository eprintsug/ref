package EPrints::Plugin::Screen::REF;
...
sub can_be_viewed
{
	...
	return 0 unless( $self->{session}->current_user->exists_and_set( 'ref_uoa' ) || $self->{session}->current_user->exists_and_set( 'ref_uoa_role' ) );
...
sub can_select
{
	...
	return 0 if
		...
		!($self->{processor}->{role}->exists_and_set( 'ref_uoa' ) );	#|| $self->{processor}->{role}->exists_and_set( 'ref_uoa_role' ) );
...
sub render_warnings
{
	...
	if( !$self->{processor}->{role}->exists_and_set( 'ref_uoa' ) )
	{
...
sub current_benchmark
{
	...
	return undef unless( $self->{session}->config( 'ref_enabled' ) );
	...
	if( $id )
	{
		$benchmark = $self->{session}->dataset( "ref_benchmark" )->dataobj( $id );
	}
	$benchmark = EPrints::DataObj::REFBenchmark->default( $self->{session} )
...
sub user_roles
{
	...
	{
		my $list = $self->{session}->call( "ref_roles_for_user",
				$self->{session},
				$user
			);
...
sub action_change_benchmark
{
	...
	my $benchmark = $self->{session}->dataset( "ref_benchmark" )->dataobj( $benchmarkid );
...
sub render_benchmarks
{
	...
	my $current_bm = EPrints::DataObj::REFBenchmark->default( $session );
	my $current_bmid = (defined $current_bm) ? $current_bm->get_id : -1;

	$session->dataset( "ref_benchmark" )->search->map(sub {
	...
	$buttons->appendChild( $session->render_action_buttons(
		change_benchmark => $session->phrase( "ref:change_benchmark" ),
		reset_benchmark => $session->phrase( "ref:reset_benchmark" ),
	) );
...
sub render_roles
{
	...
	{
		return $session->html_phrase( 'ref:error:no_users' );
	}
	...
	$buttons->appendChild( $session->render_action_buttons(
		change_role => $session->phrase( "ref:change_role" ),
		reset_role => $session->phrase( "ref:reset_role" ),
	) );
	...
	$frag = $session->html_phrase( "ref:roles",
		change_role => $form,
...
sub render_tools
{
	...
	my $tools = $self->render_action_list_short( "ref_tools" );

	if( defined $tools && $tools->getElementsByTagName( 'input' ) )
	{
		$frag->appendChild( $self->{session}->html_phrase( 'ref:tools:header' ) );
...
sub render_selections
{
	...
	my $selections = EPrints::DataObj::REFSelection::search_by_user( $repo, $processor->{role} );
	...
	foreach my $selection ($selections->get_records)
		...
		foreach my $otherid ( @{ EPrints::DataObj::REFSelection::who_selected( $repo, $eprintid ) || [] } )
		{
			if( $otherid == $roleid )
			{
			#	$already_selected = 1;
			...
			{
				push @names, $repo->phrase( "ref:unknown_user", id => $otherid );
			}
		}
		if( scalar( @names ) > 0 )
		{
		...
		my $remove = $repo->make_element( "a", href => "#",
				onclick => "return EPJS_REF_RemoveSelection( '$roleid', '$eprintid', '$userid', '$params' );"
		);
		...
		$remove = $repo->make_element( "a",
			href => sprintf("javascript:EPJS_REF_Update('%s', '%s')",
				substr("$uri",0,-1-length($uri->query)),
				$uri->query,
			),
		);

		$remove->appendChild( $repo->html_phrase( "ref/select:remove_button" ) );

		my $qualify = $repo->render_link( $selection->get_control_url );
		$qualify->appendChild( $repo->html_phrase( "ref/select:qualify_button" ) );
