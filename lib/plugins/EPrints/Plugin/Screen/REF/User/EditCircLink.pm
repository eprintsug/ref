package EPrints::Plugin::Screen::REF::User::EditCircLink;
...
# forces the use of Screen::REF::allow (which should use Screen::allow) over Screen::Workflow::allow
sub allow
{
        my( $self, @args ) = @_;

        return $self->EPrints::Plugin::Screen::REF::allow( @args );
}

# sf2: copied from REF::user_roles (because used by REF::current_role, see a few lines above)
sub user_roles
{
        my( $self, $user ) = @_;
...
sub from
{
	...
	my $circ = EPrints::DataObj::REFCirc->new_from_user( $self->{session}, $user->get_id, 1 );
	return unless( defined $circ );	# odd error
	
	my $redirect_url = $self->{session}->config( 'userhome' )."?screen=REF::User::EditCirc&dataset=ref_circ&dataobj=".$circ->get_id;
	if( EPrints::Utils::is_set( $self->{processor}->{return_to} ) )
	{
		# note that only REF::Overview sets the return_to parameter
