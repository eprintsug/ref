package EPrints::Plugin::Screen::REF::User::EditCirc;
...
sub can_be_viewed
{
        ...
        return 0 unless( defined $self->{processor}->{dataset} && ( $self->{processor}->{dataset}->id eq 'user' || $self->{processor}->{dataset}->id eq 'ref_circ' ) );
...
sub get_user_from_circ
{
	...
	return undef unless( defined $self->{processor}->{dataobj} );
        ...
        if( $self->{processor}->{dataobj}->get_dataset_id eq 'ref_circ' )
        {
...
sub properties_from
{
	...
	$self->{processor}->{role} = EPrints::Plugin::Screen::REF::current_role( $self );
...
sub view_screen
{
	...
	return "REF::User::EditCircLinkBack";
}
