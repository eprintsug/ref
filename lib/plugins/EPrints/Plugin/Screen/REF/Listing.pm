...
sub new
{
	...
	my $self = $class->EPrints::Plugin::Screen::REF::new( %params );
...
sub can_be_viewed 
{
        ...
        my $rc = $self->EPrints::Plugin::Screen::REF::can_be_viewed;
...
sub export { shift->EPrints::Plugin::Screen::REF::export }

sub properties_from
{
	my( $self ) = @_;

	$self->EPrints::Plugin::Screen::REF::properties_from;
	$self->EPrints::Plugin::Screen::AbstractSearch::properties_from;

	$self->{processor}->{dataset} = $self->{session}->dataset( "ref_selection" );
}
...
sub redirect_to_me_url
{
	...
	{
		return $self->EPrints::Plugin::Screen::REF::redirect_to_me_url;
	}
...
sub action_select
{
	...
	$selection->select_for( $benchmark, $role->value( "ref_uoa" ) );
	$selection->commit;
...
sub render_selections
{
	...
	$selections->map(sub {
		...
		if( $self->can_select )
		{
			...
			# synchronise UoA
			if( $uoa->id ne $role->value( "ref_uoa" ) )
			{
...
sub render
{
	...
	my $is_uoa_champion = $repo->current_user->exists_and_set( 'ref_uoa_role' );
