package EPrints::Plugin::Screen::REF::REF4::Listing;
...
sub can_be_viewed
{
	...
	# main REF Admins? role = ?
	# people who have their ref_uoa_role field set
...
sub render_result_row
{
	...
	my $session = $self->{session};
	my $benchmark = $self->{processor}->{benchmark};
	my $env_ds = $session->dataset("ref_environment");
...
#	foreach my $year (qw( 2008 2009 2010 2011 2012 ))
	...
	foreach my $year (sort keys %$data)
	{
		...
		$td1->appendChild( $session->html_phrase("ref_environment_fieldopt_year_".$year) );
		...
		if ($data->{$year}->{"id"} >= 0)
		{
			...
			$edit_href->query_form(
				screen => "REF::REF4::Edit",
				dataset => "ref_environment",
				dataobj => $data->{$year}->{"id"},
				return_to => "REF::REF4::Listing",
			);
