...
sub can_be_viewed
{
	...
	my %uoa_ids = map { $_->id => undef } @{ $uoas || [] };
        foreach my $uoa_role ( @{$user->value( 'ref_uoa_role' )||[]} )
        {
...
sub users
{
	...
	my $default_benchmark = EPrints::DataObj::REFBenchmark->default( $self->{session} );
...
sub users_by_uoa
{
        ...
        return $users->reorder( "ref_uoa/name" );
}
...
sub users_by_selection
{
	...
	foreach my $uoa (@uoas)
	{
        ...
        $list = $list->reorder( "ref_uoa/name" );
...
sub current_uoas
{
	...
	# selected UoA's
        ...
        my %allowed_uoas = map { $_ => undef } @{$self->{session}->current_user->get_value( 'ref_uoa_role' ) || [] };
...
EOJ
	...
	# looking at legacy data ? i.e. former benchmarks? This is slightly more complex to deal with
        my $benchmark = $self->{processor}->{benchmark};
        my $current_benchmark = EPrints::DataObj::REFBenchmark->default( $self->{session} );
	...
	$users->map( sub {
		...
		my $user_uoa = $user->value( 'ref_uoa' );
...
sub export_plugins
{
        ...
        my @plugin_ids = $self->{session}->plugin_list(
                type => "Export",
                can_accept => "report/".$self->{processor}->{report},
                is_visible => "staff",
		is_advertised => 1,
        );
