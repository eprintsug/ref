package EPrints::Plugin::Screen::REF::Overview;
...
sub available_reports
{
	...
	my $is_uoa_champion = $user->exists_and_set( 'ref_uoa_role' );
...
sub render_user_actions
{
        ...
        my $frag;
        {
                ...
                $frag = $self->render_action_list_bar( "ref_listing_user_actions", { 
...
sub render
{
	...
	if( $is_uoa_champion && !$self->user_roles( $repo->current_user )->count && !$repo->current_user->exists_and_set( 'ref_uoa' ) )
	{
		$frag->appendChild( $self->html_phrase( 'error:nothing_to_show' ) );
		return $frag;
	}

	my $circ = EPrints::DataObj::REFCirc->new_from_user( $repo, $role->get_id, 1);
