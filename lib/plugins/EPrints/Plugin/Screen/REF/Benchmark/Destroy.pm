package EPrints::Plugin::Screen::REF::Benchmark::Destroy;
...
sub can_be_viewed
{
	...
	return $self->allow( 'ref_benchmark/delete' );
}
...
sub properties_from
{
	...
	# sf2 - need to over-ride the {processor}->{benchmark} (set by default to the current BM). Here it needs to be set to $session->param( '
        $self->{processor}->{benchmark} = $session->dataset( 'ref_benchmark' )->dataobj( $session->param( 'dataobj' ) ); 
}
...
sub action_cancel
{
	...
	$self->{processor}->{dataset} = $self->{session}->dataset( 'ref_benchmark' ) unless( defined $self->{processor}->{dataset} );
}
...
sub action_remove
{
	...
	# also need to remove all the REF Selection objects associated with this benchmark
