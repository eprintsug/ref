package EPrints::Plugin::Screen::REF::Benchmark::Copy;
...
sub action_copy
{
	...
	# sf2 - Now we need to copy all the REF_Selection objects from the current benchmark to this new one

	my $bm = EPrints::DataObj::REFBenchmark->default( $self->{session} );

	# sf2 - of course if none exists, we just return... this will redirect to the REF Benchmark workflow
	...
	my $url = $self->{session}->get_url."?screen=Workflow::Edit&dataset=ref_benchmark&dataobj=".$self->{processor}->{dataobj}->get_id;
...
sub render
{
	...
	my $bm = EPrints::DataObj::REFBenchmark->default( $session );
	unless( defined $bm )
	{
