package EPrints::Plugin::Screen::REF::Benchmark::New;
...
sub new
{
	...
	if( defined $self->{session} 
		...
		&& defined $self->{session}->{datasets}->{ref_benchmark}
		&& defined EPrints::DataObj::REFBenchmark->default( $self->{session} ) )
	{
		push @$actions_appear,	{
				place => "ref_benchmark_create",
				action => "copy",
...
{ 
	...
	return $self->allow( 'ref_benchmark/create_new' ) if( defined $ds && ($ds->base_id eq 'ref_benchmark' || $ds->base_id eq 'ref_selection' ));
	return 0;
}
...
sub action_copy
{
	...
	$self->{processor}->{screenid} = 'REF::Benchmark::Copy';
