package EPrints::Plugin::Screen::REF::Benchmark::Select;
...
{ 
	...
	return $self->allow( 'ref_benchmark/select_current' ) if( defined $ds && ($ds->base_id eq 'ref_benchmark' || $ds->base_id eq 'ref_selection' ));
	return 0;
}
...
sub action_select_current
{
	...
	EPrints::DataObj::REFBenchmark::select_as_default( $self->{session}, $selected );
...
sub render
{
	...
	# search all REF Benchmark objects
