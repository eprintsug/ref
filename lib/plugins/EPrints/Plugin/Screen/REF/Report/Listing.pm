package EPrints::Plugin::Screen::REF::Report::Listing;
...
sub can_be_viewed
{
	...
	return 1 if( defined $user && $user->exists_and_set( 'ref_uoa_role' ) );
...
sub render
{
	...
	my $ref_roles = $user->get_value( 'ref_uoa_role' );
	...
	my $select_link = $links_div->appendChild( $session->make_element( 'a', href => '#', onclick => 'return EPJS_REF_SelectAllUoAs();' ) );
	$select_link->appendChild( $session->make_text( 'Select all' ) ); 
	$links_div->appendChild( $session->make_element( 'br' ) );
	my $unselect_link = $links_div->appendChild( $session->make_element( 'a', href => '#', onclick => 'return EPJS_REF_UnselectAllUoAs();' ) );
	...
	{
		my $th = $tr->appendChild( $session->make_element( "th" ) );
		$th->appendChild( $session->html_phrase( "ref:stat_$id" ) );
	}
...
sub render_reports
{
	...
	my @reports = @{$self->{session}->config( 'ref', 'reports' ) || [] };

	my $select = $self->{session}->make_element( 'select', id => "report", name => "report" );
	my $n = 0;
	foreach my $reportid ( @reports )
	{
		my $plugin = $self->{session}->plugin( "Screen::REF::Report::$reportid" ) or next;
...
{ 
	...
	my $users = $session->dataset( 'user' )->search( filters => [
                { meta_fields => [ "ref_uoa" ], value => $subject->id, },
        ]);
