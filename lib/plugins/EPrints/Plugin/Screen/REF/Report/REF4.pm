package EPrints::Plugin::Screen::REF::Report::REF4;
...
sub export
{
	...
	my $list = $self->{session}->dataset( 'ref_environment' )->search(
		filters => [
...
sub get_ref4_data
{
	...
	{
		$session->dataset( 'ref_environment' )->search(
			filters => [
				{ meta_fields => [ 'ref_benchmarkid' ], value => $benchmark->get_id },
...
sub properties_from
{
	...
	if( $format )
	{
		...
		if( defined $plugin && $plugin->can_accept( "list/ref_environment" ) )
		{
	...
	foreach my $category (qw( income income_in_kind ))
	{
		...
		foreach my $source (sort keys %{$data->{$category}})
		{
			...
			$source_para->appendChild( $self->html_phrase("research_income_source", 
                                                                       funding_source=>$session->html_phrase("ref_environment_fieldopt_".$category."_source_".$source) ) ); 
