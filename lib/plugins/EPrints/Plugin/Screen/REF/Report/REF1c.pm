package EPrints::Plugin::Screen::REF::Report::REF1c;
...
sub users
{
		...
		[
                	{ meta_fields => [ "ref_uoa" ], value => join( " ", @uoa_ids ), merge => 'ANY' },
...
sub ajax_user
{
	...
	->map(sub {
		...
		# don't display user in REF1c report if that user isn't a Cat C staff:
