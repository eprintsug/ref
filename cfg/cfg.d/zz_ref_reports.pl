...
$c->{plugins}->{"Screen::REF::Report::REF1a"}->{params}->{validate_user} = sub {
	...
	{
		my $is_mf = "is_$type";	# eg is_fixed_term, is_secondment ... (those fields are defined in zz_ref_circ.pl)
...
$c->{plugins}->{"Screen::REF::Report::REF1c"}->{params}->{validate_user} = sub {
	...
	my $type = $user->value( 'ref_category' );
	my $circ = EPrints::DataObj::REFCirc->new_from_user( $session, $user->get_id, 1 );
	my $circds = $session->dataset( 'ref_circ' ); 
...
$c->{'ref'}->{'ref1a'}->{'mappings'} = {
	...
	isResearchFellow => "ref_circ.is_fellow",
	isEarlyCareerResearcher => "ref_circ.is_ecr",
	startDate => "user.ref_start_date",
	isOnFixedTermContract => "ref_circ.is_fixed_term",
	contractStartDate => "ref_circ.fixed_term_start",
	contractEndDate => "ref_circ.fixed_term_end",
	isOnSecondment => "ref_circ.is_secondment",
	secondmentStartDate => "ref_circ.secondment_start",
	secondmentEndDate => "ref_circ.secondment_end",
	isOnUnpaidLeave => "ref_circ.is_unpaid_leave",
	unpaidLeaveStartDate => "ref_circ.unpaid_leave_start",
	unpaidLeaveEndDate => "ref_circ.unpaid_leave_end",
	isNonUKBased => "ref_circ.is_non_uk",
	nonUKBasedText => "ref_circ.non_uk_text",
	isSensitive => "user.ref_is_sensitive",
	circumstanceExplanation => "ref_circ.circ_text",
...
$c->{'ref'}->{'ref1b'}->{'mappings'} = {
	hesaStaffIdentifier => "user.hesa",
	staffIdentifier => "user.staff_id",
	circumstanceIdentifier => "ref_circ.circ",
	earlyCareerStartDate => "ref_circ.ecr_start",
	totalPeriodOfAbsence => "ref_circ.absence",
	numberOfQualifyingPeriods => "ref_circ.qual_periodes",
	complexOutputReduction => "ref_circ.complex_reduction",
};
...
$c->{'ref'}->{'ref1c'}->{'mappings'} = {
	hesaStaffIdentifier => "user.hesa",
	staffIdentifier => "user.staff_id",
	employingOrganisation => "ref_circ.catc_org",
	jobTitle => "ref_circ.catc_jobtitle",
	explanatoryText => "ref_circ.catc_text",
};
...
$c->{'ref'}->{'ref2'}->{'mappings'} = { 
	...
	"outputIdentifier" => "ref_selection.selectionid",		# ref_selection.selectionid
	"outputType" => "ref_selection.type",				# ref_selection.type
	...
	"articleNumber" => \&ref2_article_number,			# will look up either eprints.article_number (note: not a default EPrints field) or ref_selection.article_id (added in v1.2.3)
	...
	"isPendingPublication" => "ref_selection.pending",		# ref_selection.pending (bool)
	"isDuplicateOutput" => "ref_selection.duplicate",		# ref_selection.duplicate
	"isNonEnglishLanguage" => "ref_selection.non_english",		# ref_selection.non_english (bool)
	"isInterdisciplinary" => "ref_selection.interdis",		# ref_selection.interdis (bool)
	"proposeDoubleWeighting" => \&ref2_doubleWeighting	,	# ref_selection.weight
	"doubleWeightingStatement" => "ref_selection.weight_text",	# ref_selection.weight_text
	"reserveOutput" => "ref_selection.reserve",			# ref_selection.reserve
	"hasConflictsOfInterests" => "ref_selection.has_conflicts",	# ref_selection.has_conflicts
	"conflictedPanelMembers" => "ref_selection.conflicted_members",	# ref_selection.conflicted_members
	"isOutputCrossReferred" => "ref_selection.is_xref",		# ref_selection.is_xref (bool)
	"crossReferToUoa" => \&ref2_cross_ref,				# ref_selection.xref (subject)
	"additionalInformation" => "ref_selection.details",		# ref_selection.details
	"englishAbstract" => \&ref2_abstract,				# ref_selection.abstract || eprint.abstract
	"researchGroup" => "ref_selection.research_group",		# ref_selection.research_group
	"isSensitive" => "ref_selection.sensitive",			# ref_selection.sensitive (bool)

};


# for REF2 the following objects are passed to the sub's:
# 	$objects->{ref_selection} - the current REF Selection object
...
sub ref2_outputNumber
{
	...
	my $key = $user->value( 'ref_uoa' )."-".$user->get_id;
