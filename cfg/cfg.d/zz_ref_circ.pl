...
# REF Staff Circumstances dataset

# We decided on separating this data from the "user" objects because it doesn't really relate to EPrints and will
# not be useful past REF2014.

# called when a ref_selection object is committed
$c->{set_ref_circ_automatic_fields} = sub {
	my( $circ ) = @_;
...
package EPrints::DataObj::REFCirc;

@EPrints::DataObj::REFCirc::ISA = qw( EPrints::DataObj );

sub get_dataset_id { "ref_circ" }
...
sub get_control_url { $_[0]->{session}->config( "userhome" )."?screen=REF::EditCirc&circid=".$_[0]->get_id }

=item $list = EPrints::DataObj::REFCirc::search_by_user( $session, $user )

Returns the REF Circumstance object belonging to $user
...
$c->{fields}->{ref_circ} = [] if !defined $c->{fields}->{ref_circ};
unshift @{$c->{fields}->{ref_circ}}, (
