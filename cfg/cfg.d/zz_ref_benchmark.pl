...
$c->{datasets}->{ref_benchmark} = {
	class => "EPrints::DataObj::REFBenchmark",
	sqlname => "ref_benchmark",
	datestamp => "date",
	index => 1,
...
unshift @{$c->{fields}->{ref_benchmark}},
...
push @{$c->{user_roles}->{admin}}, qw{
	+ref_benchmark/details
	+ref_benchmark/edit
	+ref_benchmark/view
	+ref_benchmark/create_new
	+ref_benchmark/copy
	+ref_benchmark/select_current
	+ref_benchmark/delete
};

# REF Benchmark Module

{
package EPrints::DataObj::REFBenchmark;

our @ISA = qw( EPrints::DataObj );

sub get_dataset_id { "ref_benchmark" }

sub control_url { $_[0]->{session}->config( "userhome" )."?screen=REF::Edit&ref_benchmarkid=".$_[0]->id }
...
=item EPrints::DataObj::REFBenchmark::select_as_default( $session, $benchmarkid );
...
sub select_as_default
{
	...
	my $search = EPrints::Search->new(
		session => $session,
		dataset => $session->dataset( 'ref_benchmark' ),
		allow_blank => 1,
	);
...
=item my $benchmark = EPrints::DataObj::REFBenchmark->default( $session );
...
sub selections
{
	...
	return $self->{session}->dataset( "ref_selection" )->search(
		filters => [
...
sub uoa_selections
{
	...
	my $dataset = $repo->dataset( "ref_selection" );
